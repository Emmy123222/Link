...
import { PaymentRequestType, PaymentItem } from '@/types/payments';
...
// Transform PaymentItem to InvoiceItem format
const transformItems = (items: PaymentItem[] | undefined | null): any[] => {
  ...
  return items.map(item => ({
    ...
    tax_rate: item.tax > 0 ? (item.tax / 100) + 1 : 1, // Convert percentage to multiplier
    amount: item.price * item.quantity,
    tax_amount: (item.price * item.quantity) * (item.tax / 100),
    totalAmount: (item.price * item.quantity) * (1 + item.tax / 100),
  }));
};
...
export const PaymentRequestProvider: React.FC<PaymentRequestProviderProps> = ({ children }) => {
  ...
  const customer = paymentRequest ? {
    ...
    items: transformItems(paymentRequest.items as PaymentItem[]),
