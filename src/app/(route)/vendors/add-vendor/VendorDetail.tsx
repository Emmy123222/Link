...
import { useCreateVendorBusiness } from "@/hooks/use-vendors";
...
export default function CustomerDetail() {
  ...
  const { formData, setCurrentStep } = useVendorForm();
  const { mutateAsync: createVendorBusiness, isPending: isCreatingVendorBusiness } = useCreateVendorBusiness();
    ...
    <div className="bg-card rounded-lg shadow-sm border p-8 overflow-auto max-h-[70vh]">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex gap-4">
          <Button
            type="button"
            onClick={() => setCurrentStep(STEPS.CONFIRM_NAME)}
            colorSchema="gray"
            variant="outline"
            className="flex-1"
            disabled={isCreatingVendorBusiness}
          >
            Back
          </Button>
          <Button
            ...
            disabled={isCreatingVendorBusiness}
          >
            {isCreatingVendorBusiness ? "Saving..." : "Save & continue"}
          </Button>
        </div>
      </form>
    </div>
